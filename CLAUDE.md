...
- Make edits and `git add` them (staged, not committed), then wait for explicit go-ahead before `git commit` — the user prefers reviewing the diff in VS Code first. Once they give the go-ahead, commit, push, open the PR, wait for CI, and merge all together in that same step — `main` requires a passing `lint-and-test` check but zero approving reviews, so once CI is green there's nothing left to pause for — rather than stopping again before the merge.
...
10. **A stdio-transport MCP server is single-use, not multi-use, in the COM sense.** The MCP host (e.g. Claude Code) launches one dedicated subprocess per connection — there's no shared instance that multiple simultaneous clients multiplex into, unlike a COM multi-use server. But it's not one-shot-per-call either: that one process instance persists and handles every sequential tool call for as long as its one connection stays open, so per-process state (a module-level cache, an open client, etc.) is safe to keep across calls within a session — it just never leaks to or from a different simultaneous consumer's own separate instance. There's no pooling or auto-respawn: if that instance dies (a crash, or something external killing the process), the host's connection needs a full reconnect before another tool call can succeed — confirmed directly with the pager server, where killing its process mid-session left `notify` unusable until the host reconnected.
