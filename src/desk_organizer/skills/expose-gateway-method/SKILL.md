...
**Event delivery timing inside a long-running handler**: the dispatcher loop is single-threaded —
it pops one queue item, dispatches it, then loops back to drain the queue. If a handler blocks for
a while (e.g. one that runs the full build pipeline), an `api.call()` invoked *during* that handler
only enqueues; it isn't actually sent until the handler returns and the loop advances, since the
same thread that would drain the queue is busy running the handler. A progress-style event meant
to arrive incrementally while the handler is still running instead arrives batched, all at once,
right when the handler finishes — silently, with no error or warning. If an event genuinely needs
to fire *during* a still-running handler, use `Gateway.call_now(id, data)` instead of `api.call()`
— it bypasses the queue and sends immediately, since the underlying send is safe to call from any
thread (fire-and-forget, no callback/response support; see geo-builder#67).
